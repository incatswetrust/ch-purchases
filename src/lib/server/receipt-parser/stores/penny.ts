...
export const pennyParser: StoreParser = {
	detect(text) {
		const upper = text.toUpperCase();
		if (upper.includes('PENNY')) return 0.95;
		if (upper.includes('SC PENNY MARKET') || upper.includes('RO14876466')) return 0.9;
		return 0;
	},
	...
	parse(text) {
		...
		return { items, total, storeName: 'Penny' };
	}
};
