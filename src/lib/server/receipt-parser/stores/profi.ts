...
export const profiParser: StoreParser = {
	detect(text) {
		const upper = text.toUpperCase();
		if (upper.includes('PROFI')) return 0.95;
		if (upper.includes('SC PROFI ROM FOOD') || upper.includes('RO14626720')) return 0.9;
		return 0;
	},
	...
	parse(text) {
		...
		return { items, total, storeName: 'Profi' };
	}
};
